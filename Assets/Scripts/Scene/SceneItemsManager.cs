...
public class SceneItemsManager : SingletonMonoBehaviour<SceneItemsManager>, ISaveable {
    ...
    private void InstantiateSceneItems(List<SceneItem> sceneItemList) {
        ...
        foreach (SceneItem sceneItem in sceneItemList) {
            Vector3 pos = new Vector3(sceneItem.position.x, sceneItem.position.y, sceneItem.position.z);
            itemGameObject = Instantiate(itemPrefab, pos, Quaternion.identity, parentItem);
    ...
    public void ISaveableStoreScene(string sceneName) {
        // Remove old scene save for gameObject if it exists
